...
<!-- Navbar End  -->
    ...
    <?php
        ...
        // SQL query to retrieve partner information
        $sql = "SELECT fname, lname, bio, Proficiency, Rating, SessionPrice FROM partner";
        $result = $conn->query($sql);

        // Display partner cards based on query results
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo '<div class="partner-card">';
                echo '<img src="user icon.png.jpg" alt="Avatar">';
                echo '<h2>' . $row["fname"] . ' ' . $row["lname"] . '</h2>';
                echo '<p>' . $row["bio"] . '</p>';
                echo '<p>' . $row["Proficiency"] . '</p>';
                echo '<p>Rating: <span class="rating">' . $row["Rating"] . '</span></p>';
                echo '<p>Session Price: $' . $row["SessionPrice"] . '/hour</p>';
                echo '<a href="details.php?fname=' . urlencode($row["fname"]) . '&lname=' . urlencode($row["lname"]) . '" target="_blank" style="color: #fff;"><button>View Details</button></a>';
                echo '</div>';
            }
        } else {
